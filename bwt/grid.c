...
void layer_grid_resize(app_t *app, layer_t *layer) {
	...
	// HERE I round up division to avoid overflows on future loops.
	// I spent a few hours debugging to find it.
	layer->grid->w = (layer->pos.size.w + layer->grid->size - 1) / layer->grid->size;
	layer->grid->h = (layer->pos.size.h + layer->grid->size - 1 )/ layer->grid->size;
