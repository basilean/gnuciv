...
layer_t * layer_grid_new(SDL_Renderer *render, uint16_t size) {
	layer_t *layer = layer_new(render);
	layer->type = LAYER_GRID;
	layer->grid = SDL_calloc(1, sizeof(layer_grid_t));
	layer->grid->color = (SDL_Color){220, 220, 220, 128};
	layer->grid->count = 0;
...
//					SDL_Log("Free: %x", widget);
	...
	for(uint16_t i = 0; i < grid->widgets; i++) {
		if(grid->widget[i] == widget) {
			SDL_free(grid->widget[i]);// TODO: widget_destroy
			continue;
		}
